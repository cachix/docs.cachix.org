---
title: "Reference"
description: "agents key contains an object of many agent-name keys and store path values to deploy."
slug: "deploy/reference.html"
---

<span id="reference" class="legacy-anchor"></span>
...
`agents` key contains an object of many `agent-name` keys and [store path](/glossary.html#term-store-path) values to deploy.
...
`store-path-to-a-rollback-script` is a [store path](/glossary.html#term-store-path) to a script that is run after each deployment.
